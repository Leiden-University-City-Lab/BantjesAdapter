...
def report_accuracy_per_volume(correct_root_folder, generated_root_folder):
    ...
    for volume in volumes:
        ...
        generated_folder = os.path.join(generated_root_folder, volume)

        if os.path.isdir(correct_folder) and os.path.isdir(generated_folder):
            print(f'\nVolume: {volume}')
            ...
            for key, (correct, total, empty_match) in key_accuracies.items():
                ...
                print(f'{key}: {key_accuracy:.2f}% ({correct}/{total}), Empty Match: {empty_match_percentage:.2f}% ({empty_match}/{total})')
...
generated_root_folder = 'correct_text'
